...
- **Public API (`solver_api.py`)** orchestrates one end-to-end solve: it loads set/MetaTFT data, builds power maps and trait stats, validates config, and calls the solver. Structured results include context, meta weighting details, trait counts, and requirement satisfaction. Errors include a decision log for debugging.
